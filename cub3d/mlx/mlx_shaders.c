...
int mlx_shaders_font(glsl_info_t *glsl)
{
  ...
  glsl->font_parma = glCreateProgram();
  glAttachShader(glsl->font_parma, glsl->font_vshader);
  glAttachShader(glsl->font_parma, glsl->font_fshader);
  glLinkProgram(glsl->font_parma);

  glGetProgramiv(glsl->font_parma, GL_LINK_STATUS, &action_ok);
  if (!action_ok) {
    fprintf(stderr, "Failed to link font shader program:\n");
    display_log(glsl->font_parma, glGetProgramiv, glGetProgramInfoLog);
    return (1);
  }
