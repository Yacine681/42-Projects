...
{
    trap = "ScavTrap ";
    std::cout << "ClapTrap std::string Default constructor called" << std::endl;
    m_name = name;
    m_hitPoints = 10;
    m_energyPoints = 10;
    m_attackDamage = 0;    
...
{  
    ...
    {
         std::cout << trap << m_name << " attacks " << target << ", causing " << m_attackDamage << " points of damage" << std::endl;
         m_energyPoints--;
    }
    else
...
void ClapTrap::takeDamage(unsigned int amount)
{
    if (!m_energyPoints || !m_hitPoints)
        return ;
    std::cout << trap << m_name << " a receipt " << amount << " points of damage" << std::endl;
...
void ClapTrap::beRepaired(unsigned int amount)
{
    if (m_energyPoints && m_hitPoints)
    {
        std::cout << trap << m_name << " a receipt " << amount << " points of heal" << std::endl;
        m_hitPoints += amount;
        m_energyPoints--;
    }
    else
